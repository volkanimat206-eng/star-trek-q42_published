...
# OVERRIDE-PATTERN
# ─────────────────────────────────────────────────────────────────────────────
# weapon_data wird normalerweise vom ShipController gesetzt – aus
# ship_data.beam_weapon_data. Damit haben alle Phaser/Pulse-Phaser desselben
# Schiffes automatisch dieselben Werte (Schaden, Range, Cooldown, Farbe).
#
# Ein Override hier auf dem Mount überschreibt die zentrale Quelle NUR für
# diesen einen Mount. Beispiel: schwacher Heck-Phaser an der Galaxy-Class.
# ─────────────────────────────────────────────────────────────────────────────
@export_group("Weapon Data Override (Optional)")
## NUR setzen wenn dieser Mount ANDERE Werte braucht als die zentralen aus
## ship_data.beam_weapon_data. Beispiel: schwacher Heck-Phaser einer Galaxy.
## Leer lassen → ShipController setzt automatisch ship_data.beam_weapon_data ein.
@export var weapon_data_override: BeamWeaponData = null

@export_group("Audio")
## Alle Audio-Parameter als Resource – z.B. res://resources/audio/beam_weapons/audio_phaser_type1.tres
@export var audio_data: BeamWeaponAudioData = null
...
## weapon_data_override (Sonderfall) oder aus ship_data.beam_weapon_data (Standard).
## Nicht mehr im Inspector pro Mount – Schadens-Tuning passiert zentral.
var weapon_data: BeamWeaponData = null
...
func _ready():
	...
	# WICHTIG: _connect_weapon_node ist deferred – läuft NACH ShipController._ready(),
	# d.h. weapon_data wurde inzwischen vom Override-Pattern gesetzt.
...
func _connect_weapon_node():
	...
	if weapon_data:
		...
		push_warning("[WeaponMount] %s: weapon_data nicht gesetzt – ShipController hat weder Override noch zentrales beam_weapon_data geliefert!" % name)

	weapon_instance._initialize_nodes()
	weapon_instance.local_target_height = mouse_target_height

	# Audio-Resource auf Waffen-Instanz übertragen
	_apply_audio_data_to_weapon(weapon_instance)
...
func fire_at(target_pos: Vector3, weapon_range: float = INF, freeze_beam: bool = false, tracking_node: Node3D = null) -> bool:
	...
	# ── Audio Fire-Debug ──────────────────────────────────────────────────────
	if debug_arc and audio_data:
		print("[WeaponMount:%s] 🔫 fire_at() | charge=%s | fire=%s" % [
		name,
		audio_data.charge_sound.resource_path if audio_data.charge_sound else "NULL",
		audio_data.fire_sound.resource_path   if audio_data.fire_sound   else "NULL",
	])
...
# ─────────────────────────────────────────────────────────────────────────────
# AUDIO RESOURCE HELPER
# ─────────────────────────────────────────────────────────────────────────────

func _apply_audio_data_to_weapon(target: Node3D) -> void:
	if not audio_data:
		push_warning("[WeaponMount:%s] audio_data nicht gesetzt – kein Sound!" % name)
		return

	if "charge_sound"                in target: target.charge_sound                = audio_data.charge_sound
	if "fire_sound"                  in target: target.fire_sound                  = audio_data.fire_sound
	if "charge_volume_offset_db"     in target: target.charge_volume_offset_db     = audio_data.charge_volume_offset_db
	if "fire_volume_offset_db"       in target: target.fire_volume_offset_db       = audio_data.fire_volume_offset_db
	if "distance_attenuation_strength" in target: target.distance_attenuation_strength = audio_data.distance_attenuation_strength
	if "max_distance"                in target: target.max_distance                = audio_data.max_distance
	if "no_distance_attenuation"     in target: target.no_distance_attenuation     = audio_data.no_distance_attenuation
	if "attenuation_filter_cutoff_hz" in target: target.attenuation_filter_cutoff_hz = audio_data.attenuation_filter_cutoff_hz
	if "charge_fade_out_time"        in target: target.charge_fade_out_time        = audio_data.charge_fade_out_time
	if "fire_fade_out_time"          in target: target.fire_fade_out_time          = audio_data.fire_fade_out_time
	if "charge_fade_curve"           in target: target.charge_fade_curve           = audio_data.charge_fade_curve
	if "fire_fade_curve"             in target: target.fire_fade_curve             = audio_data.fire_fade_curve

	if debug_arc:
		print("[WeaponMount:%s] 🔊 audio_data angewendet: charge=%s | fire=%s" % [
			name,
			audio_data.charge_sound.resource_path if audio_data.charge_sound else "❌",
			audio_data.fire_sound.resource_path   if audio_data.fire_sound   else "❌",
		])
