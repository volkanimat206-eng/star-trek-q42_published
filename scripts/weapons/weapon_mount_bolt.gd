...
# OVERRIDE-PATTERN
# ─────────────────────────────────────────────────────────────────────────────
# bolt_weapon_data wird normalerweise vom ShipController gesetzt – aus
# ship_data.bolt_weapon_data. Damit haben alle Wing-Disruptoren desselben
# Schiffes automatisch dieselben Werte (Schaden, Range, Salvo, Cooldown, Farbe).
# ─────────────────────────────────────────────────────────────────────────────
@export_group("Bolt Data Override (Optional)")
## NUR setzen wenn dieser Mount ANDERE Werte braucht als die zentralen aus
## ship_data.bolt_weapon_data. Beispiel: schwächerer Sekundär-Disruptor.
## Leer lassen → ShipController setzt automatisch ship_data.bolt_weapon_data ein.
@export var bolt_weapon_data_override: BoltWeaponData = null

@export_group("Audio")
## Alle Audio-Parameter als Resource – z.B. res://resources/audio/bolt_weapons/audio_disruptor_bolt.tres
@export var audio_data: BoltWeaponAudioData = null
...
## bolt_weapon_data_override (Sonderfall) oder aus ship_data.bolt_weapon_data.
## Nicht mehr im Inspector pro Mount – Schadens-Tuning passiert zentral.
var bolt_weapon_data: BoltWeaponData = null
...
func _ready() -> void:
	...
	# Validation und Status-Print verzögert: bolt_weapon_data wird vom
	# ShipController via Override-Pattern gesetzt, aber dessen _ready() läuft
	# NACH unserem. Mit call_deferred sehen wir die finalen Werte.
	call_deferred("_post_setup_validation")


func _post_setup_validation() -> void:
	if not bolt_weapon_data:
		push_warning("[WingDisruptorMount|%s] bolt_weapon_data nicht gesetzt – ShipController hat weder Override noch zentrales bolt_weapon_data geliefert!" % name)
		return

	var data_name: String = bolt_weapon_data.weapon_name
...
func _setup_audio() -> void:
	if not audio_data or not audio_data.fire_sound:
		push_warning("[WingDisruptorMount|%s] audio_data.fire_sound fehlt." % name)
		return
	...
	if not _audio_player or not is_instance_valid(_audio_player):
		...
		_audio_player.stream = audio_data.fire_sound
		_audio_player.bus    = "Weapons"
		add_child(_audio_player)

	print("[WingDisruptorMount|%s] 🔊 Audio bereit | fire=%s | player=%s" % [
		name,
		audio_data.fire_sound.resource_path,
		_is_player_ship,
	])

func _play_fire_sound() -> void:
	if not _audio_player or not audio_data or not audio_data.fire_sound:
		return
	...
	_audio_player.volume_db = audio_data.fire_volume_offset_db

	if audio_data.no_distance_attenuation or _is_player_ship:
		...
		_audio_player.max_distance      = audio_data.max_distance
		_audio_player.unit_size         = 1.0 / max(0.1, audio_data.distance_attenuation_strength)

	_audio_player.attenuation_filter_cutoff_hz = audio_data.attenuation_filter_cutoff_hz
	_audio_player.play()

	if audio_data.sound_fade_out_time > 0.0:
		var tween := create_tween()
		tween.tween_property(_audio_player, "volume_db", -80.0, audio_data.sound_fade_out_time)\
